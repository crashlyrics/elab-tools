...
function MaterializingClaim() {
  ...
  const animationVersion = useMemo(() => "canvas-dither-cloud-v18", []);
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      const textReveal = clamp((elapsed - 4550) / 1950, 0, 1);
      const fadeOut = clamp((elapsed - 8150) / 650, 0, 1);
      ...
      const solidAlpha = Math.pow(textReveal, 1.95) * 0.96;
      if (solidAlpha > 0) {
