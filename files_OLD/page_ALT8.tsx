  ...
  for (let y = 25; y <= 135; y += stepY) {
    for (let x = 56; x <= 944; x += stepX) {
      ...
      grains.push({
        x: x + jitterX,
        y: y + jitterY,
        rx: 0.34 + sizeBias * 0.44,
        ry: 0.24 + random() ** 1.7 * 0.38,
        finalRx: 5.7 + random() * 1.0,
        finalRy: 4.7 + random() * 0.95,
        rotation: -58 + random() * 116,
        delay: random() ** 3.1 * 260 + normalizedX * 48,
        duration: 4200 + random() * 520,
        driftX: 6 + random() * 7,
...
function MaterializingClaim() {
  return (
    <div
      ...
      data-animation-version="fast-clustered-condense-v13"
    >
      <svg
        className="claim-svg"
        ...
        <defs>
          ...
          >
            {CLAIM_GRAINS.map((grain, index) => {
              ...
              const middleX = grain.x - grain.driftX * 0.28;
              const middleY = grain.y + grain.driftY * 0.28;
              ...
              return (
                ...
                >
                  <ellipse cx={startX} cy={startY} rx="0" ry="0" fill="white">
                    ...
                    <animate
                      attributeName="rx"
                      values={`0;${grain.rx.toFixed(2)};${(grain.rx * 1.65).toFixed(2)};${grain.finalRx.toFixed(2)}`}
                    ...
                    <animate
                      attributeName="ry"
                      values={`0;${grain.ry.toFixed(2)};${(grain.ry * 1.55).toFixed(2)};${grain.finalRy.toFixed(2)}`}
                      keyTimes="0;0.14;0.56;1"
                      begin={`${grain.delay.toFixed(0)}ms`}
                      dur={`${grain.duration.toFixed(0)}ms`}
                      fill="freeze"
