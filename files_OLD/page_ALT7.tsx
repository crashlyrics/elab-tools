...
function createGrains(): Grain[] {
  ...
  for (let y = 25; y <= 135; y += stepY) {
    for (let x = 56; x <= 944; x += stepX) {
      const jitterX = (random() - 0.5) * stepX * 0.52;
      const jitterY = (random() - 0.5) * stepY * 0.52;
      const sizeBias = random() ** 1.8;
      const normalizedX = (x - 56) / (944 - 56);

      grains.push({
        x: x + jitterX,
        y: y + jitterY,
        rx: 0.26 + sizeBias * 0.56,
        ry: 0.2 + random() ** 1.8 * 0.48,
        finalRx: 6.3 + random() * 1.45,
        finalRy: 5.15 + random() * 1.25,
        rotation: -62 + random() * 124,
        delay: random() ** 1.55 * 620 + normalizedX * 170,
        duration: 4300 + random() * 900,
        driftX: 10 + random() * 18,
        driftY: -3.4 + random() * 6.8,
      });
    }
  }
...
function MaterializingClaim() {
  return (
    <div
      ...
      data-animation-version="svg-frozen-grains-v12"
    >
      <svg
        className="claim-svg"
        ...
        <defs>
          ...
          >
            {CLAIM_GRAINS.map((grain, index) => {
              ...
              const middleX = grain.x - grain.driftX * 0.38;
              const middleY = grain.y + grain.driftY * 0.38;
              ...
              return (
                ...
                >
                  <ellipse cx={startX} cy={startY} rx="0" ry="0" fill="white">
                    ...
                    <animate
                      attributeName="rx"
                      values={`0;${grain.rx.toFixed(2)};${(grain.rx * 1.18).toFixed(2)};${grain.finalRx.toFixed(2)}`}
                      keyTimes="0;0.28;0.6;1"
                      begin={`${grain.delay.toFixed(0)}ms`}
                      dur={`${grain.duration.toFixed(0)}ms`}
                      fill="freeze"
                    />
                    <animate
                      attributeName="ry"
                      values={`0;${grain.ry.toFixed(2)};${(grain.ry * 1.16).toFixed(2)};${grain.finalRy.toFixed(2)}`}
                      keyTimes="0;0.28;0.6;1"
                      begin={`${grain.delay.toFixed(0)}ms`}
                      dur={`${grain.duration.toFixed(0)}ms`}
                      fill="freeze"
