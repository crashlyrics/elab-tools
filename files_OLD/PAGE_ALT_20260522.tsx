...
type Unit = "g" | "kg" | "ml" | "l" | "TL" | "EL" | "Stk." | "Prise" | "Bund" | "Zehen" | "Einh.";

type PurchaseRounding = "none" | "whole";
...
type RecipeMode = "custom" | "template";
type TemplateId = "kartoffelgratin" | "bolognese";
type LossMode = "aufschlag" | "schwund";
type PlanningMode = "standard" | "purchase";

type CalculatedIngredient = Ingredient & {
  scaled: number;
...
function formatCompactUnitAmount(value: number, unit: Unit) {
...
function calculateIngredient(item: Ingredient, factor: number, lossMode: LossMode): CalculatedIngredient {
...
function calculatePurchaseOrder(item: CalculatedIngredient): PurchaseOrder | null {
...
function MetricChip({ label, value, icon }: { label: string; value: string; icon: "factor" | "list" }) {
  return (
...
const unitOptions: Unit[] = ["g", "kg", "ml", "l", "TL", "EL", "Stk.", "Prise", "Bund", "Zehen", "Einh."];

const ingredientGridColumns =
  "grid-cols-[minmax(12.45rem,0.95fr)_4.8rem_1.25rem_4.6rem_4.2rem_1.25rem_7.4rem_1.25rem]";

const customInitialIngredients: Ingredient[] = [
  {
    name: "Neue Zutat",
    base: 0,
    unit: "kg",
...
const recipeTemplates: Record<TemplateId, RecipeTemplate> = {
  kartoffelgratin: {
    id: "kartoffelgratin",
    ...
    ingredients: [
      {
        ...
        loss: 2,
        demandUnit: "kg",
        purchase: { orderUnit: "kg", packageSize: 1, packageSizeUnit: "kg", rounding: "none" },
      },
      {
        name: "Zwiebeln",
        base: 1.2,
...
const initialIngredients: Ingredient[] = defaultTemplate.ingredients;
...
export default function ElabToolsGebindeMockup() {
  ...
  const applyTemplate = (templateId: TemplateId) => {
    ...
    setOriginalIngredients(template.ingredients);
                  ...
                  <div className="min-w-[640px]">
                    ...
                    <div className="space-y-2">
                      {calculatedIngredients.map((item, index) => {
                        ...
                        const changedFromTemplate = recipeMode === "template" && original;
          ...
          <aside className="h-fit rounded-[1.35rem] bg-[#2c3e4a] p-6 shadow-[0_24px_60px_rgba(20,30,40,0.35)] ring-1 ring-[#314754] xl:sticky xl:top-8">
            ...
            <div className="space-y-3">
              {purchaseOrders.map(({ item, order }, index) => (
                <div key={`${item.name}-purchase-${index}`} className="rounded-[1rem] bg-[#344a57] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_8px_18px_rgba(0,0,0,0.25)] ring-1 ring-[#3c5563]">
...
Bestellung: ${formatAmount(order.orderQty)} ${order.orderUnit}`
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-2">
              ...
              <button className="rounded-[1rem] bg-[#3c5563] px-4 py-3 text-sm font-medium text-slate-100 ring-1 ring-[#4a6473] shadow-[0_10px_24px_rgba(0,0,0,0.25)] transition hover:-translate-y-[1px]">Kopieren</button>
            </div>
          </aside>
        </main>
      </div>
